rule("carven.build")
    ...
    on_buildcmd_file(function (target, batchcmds, sourcefile_cv, opt)
        local carven_pkg = target:pkg("carven")
        local carven = assert(carven_pkg, "please add_packages(\"carven\") before add_rules(\"@carven/carven\")")
        local carven_program = path.join(carven:installdir(), "bin", is_host("windows") and "carven.exe" or "carven")
